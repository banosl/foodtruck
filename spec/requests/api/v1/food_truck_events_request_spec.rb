...
RSpec.describe 'endpoints for food truck event creation/updating' do
  ...
  it 'can create a new event for a foodtruck given appropriate data' do
    ...
    expect(new_event[:attributes][:event_date]).to eq(Date.today)
    expect(new_event[:attributes][:latitude]).to eq(123)
    expect(new_event[:attributes][:longitude]).to eq(123)
    expect(new_event[:attributes][:start_time]).to eq(Time.now)
    expect(new_event[:attributes][:end_time]).to eq(Time.now + 2.hours)
